...
select order_id, customer_id, freight, ship_country from orders where ship_country like 'P%' order by freight descLIMIT 10
...
select country, count(supplier_id) from suppliers group by supplier_id
...
select sum(freight) from orders where ship_region is not null group by ship_country having sum(freight) > 2750 order by sum(freight) desc
